import Reduce
import Parser # graphql parser

defmodule GraphqlToCypher do

#   def parse(query, cb) do
#     json = parser.parse(`{ ${query} }`).fields[0]
#     errorOccured
#
#     error = (err) => {
...
#
#       if (root !== parent) {
#         relationship = root.params.filter((x) => x.name === 'relationship')[0]
#         if (!relationship) return error(`missing relationship parameter for ${name}`)
#         relationshipValue = relationship.value.value
...
#           relationShip = entity.parent ? `, __${alias}r` : ''
...
#
# function param (item) {
#   value = item.type === 'Literal' ? item.value : `{${item.name}}`
